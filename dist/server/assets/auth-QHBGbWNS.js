...
function isDynamicBaseURLConfig(config2) {
	return (
		...
		config2 !== null &&
		"allowedHosts" in config2 &&
		Array.isArray(config2.allowedHosts)
	);
}
...
function resolveDynamicBaseURL(config2, request, basePath) {
	...
	if (config2.allowedHosts.some((pattern) => matchesHostPattern(host, pattern)))
		return withPath(
			`${getProtocolFromRequest(request, config2.protocol)}://${host}`,
			basePath,
		);
	if (config2.fallback) return withPath(config2.fallback, basePath);
	throw new BetterAuthError(
		`Host "${host}" is not in the allowed hosts list. Allowed hosts: ${config2.allowedHosts.join(", ")}. Add this host to your allowedHosts config or provide a fallback URL.`,
	);
}
...
function parseAccountOutput(options, account2) {
	const {
		...
		password: _password,
		...rest
	} = filterOutputFields(account2, getFields(options, "account", "output"));
	return rest;
}
...
function parseAccountInput(options, account2) {
	return parseInputData(account2, {
...
const getSessionQuerySchema$1 = z.optional(
	z.object({
...
async function generateGenericState(c, stateData, settings) {
	...
	if (c.context.oauthConfig.storeStateStrategy === "cookie") {
		...
		const stateCookie2 = c.context.createAuthCookie("oauth_state", {
			maxAge: 600,
		});
...
const { get: getOAuthState, set: setOAuthState } = defineRequestState(
	() => null,
);
...
async function parseState(c) {
	...
	try {
		...
		if (
			error2 instanceof StateError &&
			error2.code === "state_security_mismatch"
		)
...
function isAPIError(error2) {
	return (
		error2 instanceof APIError$1 ||
		error2 instanceof APIError ||
...
const logBackwardCompatWarning = deprecate(
	function logBackwardCompatWarning2() {},
...
async function validateOrigin(ctx, forceValidate = false) {
	...
	if (Array.isArray(skipOriginCheck))
		try {
			...
			if (skipOriginCheck.some((skipPath) => currentPath.startsWith(skipPath)))
				return;
		} catch {}
...
function shouldRateLimit(max, window2, rateLimitData) {
	...
	return (
		now2 - rateLimitData.lastRequest < windowInMs && rateLimitData.count >= max
	);
}
...
async function onResponseRateLimit(req, ctx) {
	...
	else if (now2 - data.lastRequest > currentWindow * 1e3)
		await storage.set(
			key,
			{
				...data,
...
function getStorageOption(identifier, config2) {
	...
	if (typeof config2 === "object" && "default" in config2) {
		if (config2.overrides) {
			for (const [prefix, option] of Object.entries(config2.overrides))
...
const createInternalAdapter = (adapter, ctx) => {
	...
	return {
					...
					? {
							fn: async (sessionData) => {
								...
								if (currentList) {
									...
									list = list.filter(
										(session2) =>
											session2.expiresAt > now2 &&
											session2.token !== data.token,
									);
								}
								const sorted = [
									...list,
							...
							async fn(data) {
								...
								if (sessionTTL > 0) {
									...
									const sorted = (listRaw ? safeJSONParse(listRaw) || [] : [])
										.filter(
											(s) => s.token !== sessionToken && s.expiresAt > now2,
										)
										.concat([
											{
												token: sessionToken,
									...
									if (furthestSessionExp && furthestSessionExp > now2)
										await secondaryStorage.set(
											listKey,
											JSON.stringify(sorted),
											getTTLSeconds(furthestSessionExp, now2),
										);
				...
				if (data) {
					...
					if (currentList) {
						...
						const filtered = list.filter(
							(session3) =>
								session3.expiresAt > now2 && session3.token !== token,
						);
		...
		findVerificationValue: async (identifier) => {
			...
			if (!verification2.length && storageOption && storageOption !== "plain")
				verification2 = await findByIdentifier(identifier);
		...
		async (ctx) => {
			...
			try {
				...
				if (
					...
					if ((session3.version || "1") !== expectedVersion)
						expireCookie(ctx, ctx.context.authCookies.sessionData);
					else {
						const cachedSessionExpiresAt = new Date(session3.session.expiresAt);
						if (
							...
							const shouldSkipSessionRefresh2 =
				...
				if (
					!session2 ||
					session2.session.expiresAt < /* @__PURE__ */ new Date()
				) {
				...
				if (dontRememberMe || ctx.query?.disableRefresh) {
					const parsedSession2 = parseSessionOutput(
						ctx.context.options,
						session2.session,
					);
					const parsedUser2 = parseUserOutput(
						ctx.context.options,
						session2.user,
					);
					return ctx.json({
				...
				const shouldBeUpdated =
					session2.session.expiresAt.valueOf() -
						expiresIn * 1e3 +
						updateAge * 1e3 <=
					Date.now();
...
const freshSessionMiddleware = createAuthMiddleware(async (ctx) => {
	...
	const lastUpdated = new Date(
		session2.session.updatedAt || session2.session.createdAt,
	).getTime();
	...
	createAuthEndpoint(
		...
		async (ctx) => {
			try {
				...
				return ctx.json(
					activeSessions.map((session2) =>
						parseSessionOutput(ctx.context.options, session2),
					),
				);
			} catch (e) {
...
const revokeSession = createAuthEndpoint(
	"/revoke-session",
	{
		method: "POST",
		body: z.object({
			token: z.string().meta({ description: "The token to revoke" }),
		}),
	...
	async (ctx) => {
		...
		if (
			...
			try {
				...
				ctx.context.logger.error(
					error2 && typeof error2 === "object" && "name" in error2
						? error2.name
						: "",
					error2,
				);
...
const revokeOtherSessions = createAuthEndpoint(
	...
	async (ctx) => {
		...
		const otherSessions = (
			await ctx.context.internalAdapter.listSessions(session2.user.id)
		)
			.filter((session3) => {
				return session3.expiresAt > /* @__PURE__ */ new Date();
			})
			.filter(
				(session3) => session3.token !== ctx.context.session.session.token,
			);
		await Promise.all(
			otherSessions.map((session3) =>
				ctx.context.internalAdapter.deleteSession(session3.token),
			),
		);
		return ctx.json({ status: true });
	},
...
const linkSocialAccount = createAuthEndpoint(
	...
	async (c) => {
		...
		if (c.body.idToken) {
			...
			if (
				(await c.context.internalAdapter.findAccounts(session2.user.id)).find(
			...
			if (
				linkingUserInfo.user.email?.toLowerCase() !==
					session2.user.email.toLowerCase() &&
...
const unlinkAccount = createAuthEndpoint(
	...
	async (ctx) => {
		...
		const accountExist = accounts.find((account2) =>
			accountId
				? account2.accountId === accountId && account2.providerId === providerId
				: account2.providerId === providerId,
		);
		if (!accountExist)
	...
	async (ctx) => {
		...
		try {
			...
			const accessTokenExpired =
				account2.accessTokenExpiresAt &&
				new Date(account2.accessTokenExpiresAt).getTime() - Date.now() < 5e3;
			if (
				...
				const updatedData = {
					...
					refreshTokenExpiresAt:
						newTokens?.refreshTokenExpiresAt ?? account2.refreshTokenExpiresAt,
			...
			const accessTokenExpiresAt = (() => {
				...
				if (account2.accessTokenExpiresAt) {
					if (typeof account2.accessTokenExpiresAt === "string")
						return new Date(account2.accessTokenExpiresAt);
			...
			const tokens = {
				accessToken:
					...
					(await decryptOAuthToken(account2.accessToken ?? "", ctx.context)),
				accessTokenExpiresAt,
...
const refreshToken = createAuthEndpoint(
	...
	async (ctx) => {
		...
		if (accountData && providerId === accountData.providerId)
			refreshToken2 = accountData.refreshToken ?? void 0;
		else refreshToken2 = account2.refreshToken ?? void 0;
		if (!refreshToken2)
			throw APIError.from("BAD_REQUEST", {
		...
		try {
			...
			const resolvedRefreshTokenExpiresAt =
				tokens.refreshTokenExpiresAt ?? account2.refreshTokenExpiresAt;
			if (account2.id) {
				const updateData = {
					...(account2 || {}),
...
const accountInfo = createAuthEndpoint(
	...
	async (ctx) => {
		...
		if (!account2 || account2.userId !== ctx.context.session.user.id)
			throw APIError.from("BAD_REQUEST", BASE_ERROR_CODES.ACCOUNT_NOT_FOUND);
		const provider = await getAwaitableValue(ctx.context.socialProviders, {
			value: account2.providerId,
		});
		if (!provider)
			throw APIError.from("INTERNAL_SERVER_ERROR", {
				message: `Provider account provider is ${account2.providerId} but it is not configured`,
...
const sendVerificationEmail = createAuthEndpoint(
	...
	async (ctx) => {
		...
		if (!session2) {
			...
			if (!user2 || user2.user.emailVerified) {
...
const verifyEmail = createAuthEndpoint(
	...
	async (ctx) => {
		...
		if (!user2) return redirectOnError(BASE_ERROR_CODES.USER_NOT_FOUND);
		if (parsed.updateTo) {
			const session2 = await getSessionFromCtx(ctx);
			if (session2 && session2.user.email !== parsed.email)
	...
	if (dbUser) {
		const linkedAccount =
			...
			dbUser.accounts.find(
				(acc) =>
					acc.providerId === account2.providerId &&
					acc.accountId === account2.accountId,
			);
		if (!linkedAccount) {
			...
			if (
				(!(
					...
					c.context.trustedProviders.includes(account2.providerId)
				) &&
...
async function validatePassword(ctx, data) {
	...
	)?.find((account2) => account2.providerId === "credential");
...
const verifyPassword = createAuthEndpoint(
	"/verify-password",
	{
		method: "POST",
		body: z.object({
...
const updateSessionBodySchema = z.record(
	z.string().meta({ description: "Field name must be a string" }),
	z.any(),
);
const updateSession = () =>
	createAuthEndpoint(
...
const changePassword = createAuthEndpoint(
	...
	async (ctx) => {
		const {
			newPassword,
			currentPassword,
			revokeOtherSessions: revokeOtherSessions2,
		} = ctx.body;
		const session2 = ctx.context.session;
		const minPasswordLength = ctx.context.password.config.minPasswordLength;
		...
		const account2 = (
			await ctx.context.internalAdapter.findAccounts(session2.user.id)
		).find(
			(account3) => account3.providerId === "credential" && account3.password,
		);
...
async function getAdapter(options) {
	return getBaseAdapter(options, async (opts) => {
		const { createKyselyAdapter: createKyselyAdapter2 } =
			await import("./index-C6hDhOht.js");
		const { kysely, databaseType, transaction } =
			await createKyselyAdapter2(opts);
		if (!kysely)
			throw new BetterAuthError("Failed to initialize database adapter");
		const { kyselyAdapter } = await import("./index-C6hDhOht.js");
...
function matchType(columnDataType, fieldType, dbType) {
	...
	return (
		Array.isArray(fieldType)
			? types2["string"].map((t) => t.toLowerCase())
			: types2[fieldType].map((t) => t.toLowerCase())
...
async function getMigrations(config2) {
	...
	let { kysely: db2, databaseType: dbType } =
		await createKyselyAdapter(config2);
	if (!dbType) {
		logger2.warn(
	...
	if (dbType === "postgres") {
		...
		try {
			...
			logger2.debug(
				`Could not verify schema existence: ${error2 instanceof Error ? error2.message : String(error2)}`,
			);
		}
	}
	...
	if (dbType === "postgres")
		try {
			...
			logger2.warn(
				`Could not filter tables by schema. Using all discovered tables. Error: ${error2 instanceof Error ? error2.message : String(error2)}`,
			);
		}
	const toBeCreated = [];
	const toBeAdded = [];
...
function buildSecretConfig(secrets, legacySecret) {
	...
	return {
		...
		legacySecret:
			legacySecret && legacySecret !== DEFAULT_SECRET ? legacySecret : void 0,
	};
}
...
async function createAuthContext(adapter, options, getDatabaseType) {
	...
	if (
		...
		logger2.error(`[better-auth] Misconfiguration detected.
...
createAuthMiddleware(async (ctx) => {
	const session2 = await getSessionFromCtx(ctx);
	if (!session2) throw APIError.fromStatus("UNAUTHORIZED");
	return { session: session2 };
});
z.object({
...
z.object({
	...
	role: z
		.union([
			z.string().meta({ description: "The role of the user" }),
			z.array(z.string().meta({ description: "The roles of user" })),
		])
		.optional()
		.meta({
...
z.object({
	...
	banReason: z
		.string()
		.meta({ description: "The reason for the ban" })
		.optional(),
	banExpiresIn: z
		.number()
...
z.object({ userId: z.coerce.string().meta({ description: "The user id" }) });
z.object({
	sessionToken: z.string().meta({ description: "The session token" }),
});
z.object({ userId: z.coerce.string().meta({ description: "The user id" }) });
z.object({ userId: z.coerce.string().meta({ description: "The user id" }) });
z.object({
	newPassword: z
		...
		.meta({ description: "The new password" }),
	userId: z.coerce
		.string()
		.nonempty("userId cannot be empty")
		.meta({ description: "The user id" }),
});
z.object({
	userId: z.coerce
		.string()
	...
	return {
		...
		hooks: {
			after: [
				{
					matcher: (ctx) =>
						ctx.path === "/multi-session/list-device-sessions" && false,
		...
		endpoints: {
			getSession: createAuthEndpoint(
				...
				async (ctx) => {
					...
					if (!session2?.response) return ctx.json(null);
					const fnResult = await fn(session2.response, ctx);
					for (const cookieStr of session2.headers.getSetCookie())
						parseSetCookieHeader(cookieStr).forEach((attrs, name) => {
					...
					session2.headers.delete("set-cookie");
					session2.headers.forEach((value, key) => {
...
z.object({
	client_id: z
		.string()
		.meta({ description: "The client ID of the application" }),
	scope: z
		.string()
		.meta({ description: "Space-separated list of scopes" })
		.optional(),
});
z.object({
	error: z
...
z.object({
	grant_type: z
		...
		.meta({ description: "The grant type for device flow" }),
...
createAuthEndpoint(
	"/device",
	{
		method: "GET",
		query: z.object({
			user_code: z.string().meta({ description: "The user code to verify" }),
		}),
		error: z.object({
...
createAuthEndpoint(
	"/device/approve",
	{
		method: "POST",
		body: z.object({
			userCode: z.string().meta({ description: "The user code to approve" }),
		}),
		error: z.object({
			error: z
				.enum([
	...
	async (ctx) => {
		...
		if (deviceCodeRecord.userId && deviceCodeRecord.userId !== session2.user.id)
...
createAuthEndpoint(
	"/device/deny",
	{
		method: "POST",
		body: z.object({
			userCode: z.string().meta({ description: "The user code to deny" }),
		}),
		error: z.object({
			error: z
				.enum([
	...
	async (ctx) => {
		...
		await ctx.context.adapter.update({
			...
			update: {
				status: "denied",
				userId: deviceCodeRecord.userId || session2.user.id,
			},
		});
		return ctx.json({ success: true });
	},
...
const timeStringSchema = z.custom(
	(val) => {
...
z.object({
	email: z
		.string({})
		.meta({ description: "Email address the OTP was sent to" }),
	type: z.enum(types).meta({
		required: true,
		description: "Type of the OTP",
	}),
...
z.object({
	email: z
		.string()
		.meta({ description: "Email address to reset the password" }),
...
z.object({
	newEmail: z
		.string()
		.meta({ description: "New email address to send the OTP" }),
	otp: z
		.string()
		.optional()
		.meta({
...
z.object({
	newEmail: z
		.string()
		.meta({ description: "New email address to verify and change to" }),
...
z.object({
	...
	callbackURL: z
		.string()
		.meta({ description: "The URL to redirect to after sign in" })
		.optional(),
	errorCallbackURL: z
		.string()
		.meta({ description: "The URL to redirect to if an error occurs" })
		.optional(),
	...
	disableRedirect: z
		.boolean()
		.meta({ description: "Disable redirect" })
		.optional(),
	scopes: z
		.array(z.string())
		.meta({
...
z.object({
	...
	error: z
		.string()
		.meta({ description: "The error message, if any" })
		.optional(),
	error_description: z
		.string()
		.meta({ description: "The error description, if any" })
		.optional(),
	state: z
		.string()
		.meta({ description: "The state parameter from the OAuth2 request" })
		.optional(),
...
z.object({
	email: z
		.email()
		.meta({ description: "Email address to send the magic link" }),
	name: z
		.string()
		.meta({
			description:
	...
	callbackURL: z
		.string()
		.meta({ description: "URL to redirect after magic link verification" })
		.optional(),
	newUserCallbackURL: z
		.string()
		.meta({
	...
	errorCallbackURL: z
		.string()
		.meta({ description: "URL to redirect after error." })
		.optional(),
});
z.object({
...
z.object({
	...
	client_name: z
		.string()
		.meta({ description: 'The name of the application. Eg: "My App"' })
		.optional(),
	client_uri: z
		.string()
		.meta({
	...
	software_statement: z
		.string()
		.meta({ description: "The software statement of the application." })
		.optional(),
});
z.object({
...
z.object({
	sessionToken: z
		.string()
		.meta({ description: "The session token to set as active" }),
});
z.object({
	sessionToken: z.string().meta({ description: "The session token to revoke" }),
});
z.object({
	callbackURL: z
		.string()
		.meta({ description: "The URL to redirect to after the proxy" }),
	profile: z
		.string()
		.optional()
		.meta({ description: "Encrypted OAuth profile data" }),
});
z.object({
	code: z.string().optional(),
	error: z.string().optional(),
...
z.object({
	token: z
		.string()
		.meta({ description: 'The token to verify. Eg: "some-token"' }),
});
defineErrorCodes({
...
z.object({
	...
	permission: z
		...
		.meta({ description: "The permission to assign to the role" }),
});
z.object({
	organizationId: z
		.string()
	...
	z.union([
		z.object({
			roleName: z
				.string()
				.nonempty()
				.meta({ description: "The name of the role to delete" }),
		}),
		z.object({
			roleId: z
				.string()
				.nonempty()
				.meta({ description: "The id of the role to delete" }),
		}),
	]),
);
z.object({
		...
		z.union([
			z.object({
				roleName: z
					.string()
					.nonempty()
					.meta({ description: "The name of the role to read" }),
			}),
			z.object({
				roleId: z
					.string()
					.nonempty()
					.meta({ description: "The id of the role to read" }),
			}),
		]),
	)
	.optional();
z.union([
	z.object({
		roleName: z
			.string()
			.nonempty()
			.meta({ description: "The name of the role to update" }),
	}),
	z.object({
		roleId: z
			.string()
			.nonempty()
			.meta({ description: "The id of the role to update" }),
	}),
]);
z.object({
	email: z
		.string()
		.meta({ description: "The email address of the user to invite" }),
	role: z
		.union([
			z.string().meta({ description: "The role to assign to the user" }),
			z.array(
				z.string().meta({ description: "The roles to assign to the user" }),
			),
		])
		.meta({
			description:
	...
	organizationId: z
		.string()
		.meta({ description: "The organization ID to invite the user to" })
		.optional(),
	resend: z
		.boolean()
		.meta({
	...
	teamId: z.union([
		z
			.string()
			.meta({ description: "The team ID to invite the user to" })
			.optional(),
		z
			.array(z.string())
			.meta({ description: "The team IDs to invite the user to" })
			.optional(),
	]),
});
z.object({
	invitationId: z
		.string()
		.meta({ description: "The ID of the invitation to accept" }),
});
z.object({
	invitationId: z
		.string()
		.meta({ description: "The ID of the invitation to reject" }),
});
z.object({
	invitationId: z
		.string()
		.meta({ description: "The ID of the invitation to cancel" }),
});
z.object({
	id: z.string().meta({ description: "The ID of the invitation to get" }),
});
z.object({
	organizationId: z
		.string()
		.meta({ description: "The ID of the organization to list invitations for" })
		.optional(),
}).optional();
z.object({
	userId: z.coerce
...
z.object({
	memberIdOrEmail: z
		.string()
		.meta({ description: "The ID or email of the member to remove" }),
	organizationId: z
		.string()
		.meta({
			description:
...
z.object({
	...
	logo: z
		.string()
		.meta({ description: "The logo of the organization" })
		.optional(),
	metadata: z
		.record(z.string(), z.any())
		.meta({ description: "The metadata of the organization" })
		.optional(),
...
z.object({
	slug: z
		.string()
		.meta({ description: 'The organization slug to check. Eg: "my-org"' }),
});
z.object({
	name: z
		.string()
		.min(1)
		.meta({ description: "The name of the organization" })
		.optional(),
	slug: z
		.string()
		.min(1)
		.meta({ description: "The slug of the organization" })
		.optional(),
	logo: z
		.string()
		.meta({ description: "The logo of the organization" })
...
z.object({
	organizationId: z
		.string()
		.meta({ description: "The organization id to delete" }),
});
z.optional(
	z.object({
		organizationId: z
			.string()
			.meta({ description: "The organization id to get" })
			.optional(),
		organizationSlug: z
			.string()
			.meta({ description: "The organization slug to get" })
			.optional(),
		membersLimit: z
			.number()
...
z.object({
	teamId: z
		.string()
		.meta({ description: `The team ID of the team to remove. Eg: "team-id"` }),
	organizationId: z
		.string()
		.meta({
...
z.object({
	teamId: z
		.string()
		.meta({ description: "The team the user should be a member of." }),
	userId: z.coerce
		.string()
		.meta({
			description:
...
z.object({
	teamId: z
		.string()
		.meta({ description: "The team the user should be removed from." }),
	userId: z.coerce
		.string()
...
z.object({
	code: z
		.string()
		.meta({ description: `A backup code to verify. Eg: "123456"` }),
	disableSession: z
		.boolean()
		.meta({ description: "If true, the session cookie will not be set." })
		.optional(),
	trustDevice: z
		.boolean()
		.meta({
...
z.object({
	code: z
		.string()
		.meta({ description: 'The otp code to verify. Eg: "012345"' }),
	trustDevice: z
		.boolean()
		.optional()
		.meta({
...
z.object({
	secret: z
		.string()
		.meta({ description: "The secret to generate the TOTP code" }),
});
...
z.object({
	...
	issuer: z
		.string()
		.meta({ description: "Custom issuer for the TOTP URI" })
		.optional(),
});
z.object({ password: z.string().meta({ description: "User password" }) });
z.object({
	...
	rememberMe: z
		.boolean()
		.meta({ description: "Remember the user session" })
		.optional(),
	callbackURL: z
		.string()
...
z.object({
	username: z.string().meta({ description: "The username to check" }),
});
...
const getUserAccount = createServerFn({
	...
	.inputValidator((session2) => session2)
	.handler(
		createSsrRpc(
			"029131a4f772755474fa44e863b4ee627ac78b88728915d7fe2e07a158849e82",
			() =>
				import("./get-user-account-BbpCmkld.js").then(
					(m) => m["getUserAccount_createServerFn_handler"],
				),
		),
	);
...
export { auth as a, account as b, db as d, getBaseURL as g };
